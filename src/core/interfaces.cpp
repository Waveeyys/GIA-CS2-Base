...
namespace interfaces {
	...
	T* capture_interface(const char* module_name, const char* interface_name) {
		...
		if (create_interface == nullptr) {
			throw std::runtime_error(std::format(
				"Failed to get CreateInterface address for module \"{}\"", module_name));
		}

		T* interface_ptr = create_interface(interface_name, nullptr);
		if (interface_ptr == nullptr) {
			throw std::runtime_error(
				std::format("Failed to capture interface \"{}\" from \"{}\"", interface_name, module_name));
		}
	...
	static void create_d3d11_resources() {
		{
			std::uint8_t* address = 
				sdk::find_pattern("rendersystemdx11.dll", "48 89 2D ? ? ? ? 48 C7 85");
		...
		IDXGISwapChain* swap_chain = swap_chain_dx11->swap_chain; // IDXGISwapChain* swap_chain = reinterpret_cast<IDXGISwapChain*>(swap_chain_dx11->swap_chain);

		if (FAILED(swap_chain->GetDevice(__uuidof(ID3D11Device), reinterpret_cast<void**>(&d3d11_device_context)))) {
			throw std::runtime_error("Failed to get ID3D11Device from swap chain.");
		}
...

		{
			...
			if (hwnd == nullptr) {
				throw std::runtime_error("Failed to get window handle from swap chain description.");
			}
		}
	...
	void create() {
		{
			std::uint8_t* address = 
				sdk::find_pattern("client.dll", "48 8B 8D ? ? ? ? 4C 8D 8F ? ? ? ? 45 33 F6");
	...
	void create_render_target() {
		...
		if (d3d11_render_target_view == nullptr) {
			throw std::runtime_error("render taget view is null.");
		}
	}
